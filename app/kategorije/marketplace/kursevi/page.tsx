...
export default async function KurseviPage() {
  let data = [];
  try {
    data = await fetchMarketplaceItems(ItemType.COURSE);
  } catch (err) {
    console.error("Error fetching courses for marketplace category:", err);
    return (
  ...
  if (!Array.isArray(data) || (data as unknown[]).length === 0) {
    return (
      <div className='min-h-screen bg-gray-50 dark:bg-gray-900 bg-grid-pattern py-16 px-4'>
        <div className='max-w-7xl mx-auto'>
          <div className='text-center py-20 opacity-0 animate-fadeInUp'>
            <div className='inline-flex items-center justify-center w-20 h-20 rounded-2xl bg-gradient-to-br from-emerald-500 to-cyan-600 mb-6 shadow-lg'>
              <svg
            ...
            <p className='text-gray-600 dark:text-gray-300'>
              Trenutno nema kurseva u ponudi. Podijeli svoj kurs sa drugima!
            </p>
            <Link 
              href={'/kategorije/marketplace/dodaj-artiklu'}
              className="mt-8 inline-block bg-gradient-to-r from-emerald-500 to-cyan-600 hover:from-emerald-600 hover:to-cyan-700 text-white font-semibold py-3 px-6 rounded-full shadow-lg transform hover:scale-105 transition-transform duration-300 animate-fadeInUp delay-400"
            >
              Dodaj svoj kurs 
            </Link>
          </div>
        ...
        <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
          <div className='text-center'>
            ...
            <p className='mt-6 text-lg sm:text-xl text-gray-600 dark:text-gray-300 max-w-3xl mx-auto opacity-0 animate-fadeInUp delay-200'>
              Pronađi kurseve za programiranje, dizajn, jezike i mnogo više
            </p>
            <div className='mt-6 inline-flex items-center gap-2 px-6 py-3 bg-white dark:bg-gray-800 rounded-full shadow-lg opacity-0 animate-fadeInUp delay-300 border border-gray-200 dark:border-gray-700'>
              ...
              <span className='font-bold text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-cyan-600'>
                {data.length}
              </span>
            </div>
            <Link 
              href={'/kategorije/marketplace/dodaj-artiklu'}
              className="mt-8 inline-block bg-gradient-to-r from-emerald-500 to-cyan-600 hover:from-emerald-600 hover:to-cyan-700 text-white font-semibold py-3 px-6 rounded-full shadow-lg transform hover:scale-105 transition-transform duration-300 opacity-0 ml-2 animate-fadeInUp delay-400"
            >
              Dodaj svoj kurs 
            </Link>
          </div>
